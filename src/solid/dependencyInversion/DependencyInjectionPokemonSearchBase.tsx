...
import type { GetPokemonInfos } from '../types/GetPokemonType'

export type PokemonSearchBaseProps = {
  searchInfoText: string
  serachUrl: string
  serachFucntion: GetPokemonInfos;
}

const PokemonSearchBase: React.FC<PokemonSearchBaseProps> = ({ searchInfoText, serachUrl, serachFucntion }) => {
  ...
  const handleSearch = async () => {
    prepareForSearch()
    try {
      const data = await serachFucntion({ searchText: searchedPokemonText, apiUrl: serachUrl })
